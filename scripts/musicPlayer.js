...
export const musicPlayerInit = () => {
  ...
  //объявление плейлиста
  const playlist = ['hello', 'flow', 'speed'];

  //индекс играющей песни
  let trackIndex = 0;
  ...
  //определение нажатой кнопки и выполнение действия
  audioNavigation.addEventListener('click', event => {
    ...
    //переключение на предыдущий трек по нажатию на кнопку
    if (target.classList.contains('audio-button__prev')) prevTrack();

    //переключение на следующий трек по нажатию на кнопку
    if (target.classList.contains('audio-button__next')) nextTrack();

  });

  //переключение трека по его окончанию
